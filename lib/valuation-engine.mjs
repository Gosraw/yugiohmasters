...
function classifyReference(term, clause, cardName) {
  ...
  // 2026-08-30 fix (Codex calibration audit, Maxx "C"): a handful of
  // real card names contain their own internal quote mark (Maxx "C"
  // is the flagship example). Its standard hard-once-per-turn clause
  // reads `...this effect of "Maxx "C"" once per turn.` - the
  // reference-extraction regex (extractQuotedReferences, a simple
  // alternating-quote scanner) isolates the FIRST quoted segment it
  // finds in that run of adjacent quote marks, which for this name
  // shape is "Maxx " (the text before the name's own embedded quote),
  // not the full "Maxx "C"" - so neither an exact cardName match NOR
  // a "this term is quoted inside cardName" check catches it. The
  // robust fix: if the CLAUSE containing the reference literally
  // contains the card's own full name, this is (overwhelmingly, by
  // real oracle-text convention) the standard "You can only use this
  // effect of "<this card's name>" once per turn" self-reference
  // pattern, regardless of which exact substring the regex's quote
  // scan isolated - never a coincidental external reference, since an
  // unrelated card's name appearing verbatim inside another card's
  // own text is not a real oracle-text convention.
  if (cardName && lowerClause.includes(cardName.toLowerCase())) {
    return { term, type: "self_reference", severity: 0 };
  }

  // Narrower companion check, kept for the (real, if rarer) case
  // where only the quoted fragment itself - not the whole clause via
  // the check above - is available to inspect: the extracted term is
  // literally quoted inside the card's own name string.
  if (cardName) {
    const lowerName = cardName.toLowerCase();
    const quotedInOwnName =
      lowerName.includes(`"${lowerTerm}"`) || lowerName.includes(`“${lowerTerm}”`);
    if (quotedInOwnName) {
      return { term, type: "self_reference", severity: 0 };
    }
  }

  // 2026-08-30 fix (Codex calibration audit, Gorz the Emissary of
  // Darkness): a card that Special Summons a Token it names itself
  // ("Special Summon 1 "Emissary of Darkness Token"...") is not
  // dependent on anything external - the quoted name is the card's
  // OWN generated Token, always available, never something the deck
  // needs to already contain. Without this check it fell through to
  // ambiguous_reference and was wrongly penalized as an external
  // dependency. Narrow and safe: only fires when the quoted term
  // itself ends in "Token" AND the same clause both creates it
  // (Special/Normal Summon) and names it as a Token - a real card
  // depending on an EXTERNAL named Token (rare, but see e.g. cards
  // that require a specific pre-existing Token on the field) would
  // not match this shape, since those reference an existing Token
  // rather than summoning a freshly named one.
  if (/token$/i.test(lowerTerm) && /special summon|normal summon/i.test(lowerClause)) {
    return { term, type: "self_reference", severity: 0 };
  }
...
// 2026-08-30 human calibration pass addition: a rough, reusable
// material-COUNT estimate (independent of specificity). "+"/"plus"
// splits the requirement into its component clauses and sums the
// leading integer found in each ("1 Fusion, Synchro, or Xyz Monster,
// plus 1 non-Tuner monster" -> 1 + 1 = 2; "3 Level 4 monsters" -> 3).
// A clause with no explicit number but that still names a monster
// requirement ("Any monster") counts as 1 rather than 0, so a
// same-shaped multi-clause requirement with one unnumbered clause
// isn't silently undercounted. This exists ONLY to let scoreCard
// distinguish "ordinary 2-material Xyz/Fusion" from "3+ material"
// summons per the calibration brief's explicit instruction ("Do not
// punish ordinary 2-material Xyz or normal Fusion equally") - it is
// not a new specificity bucket, just a count alongside the existing
// generic/constrained/named one.
function estimateMaterialCount(materialText) {
  if (!materialText) return 0;
  const segments = materialText.split(/\bplus\b|\+/i);
  let total = 0;
  let foundAny = false;
...
function parseExtraDeckMaterials(text, isExtraDeckCard) {
  if (!isExtraDeckCard) {
    return { specificity: "n/a", materialText: "", materialCount: 0, reason: "" };
  }
  ...
  if (!materialText) {
    return { specificity: "generic", materialText: "", materialCount: 0, reason: "No material text found - treated as generic rather than assumed narrow." };
  }

  const materialCount = estimateMaterialCount(materialText);
...
export function extractValuationSignals(card) {
  ...
  // 2026-08-30 fix (Codex calibration audit, Maxx "C"): the strict
  // literal "(Quick Effect)" parenthetical missed the equally common
  // real oracle-text convention "(This is a Quick Effect.)" (used by,
  // among others, Maxx "C" itself) - broadened to any occurrence of
  // the phrase "quick effect" rather than requiring that one exact
  // bracketed shape. No real card uses "quick effect" outside this
  // family of parenthetical timing markers, so this is not expected
  ...
  // 2026-08-30 human calibration pass (section 9A - "difficult tribute
  // requirements"): a plain costTribute flag treats "Tribute 1
  // monster" and "Tribute 2 monsters" identically. Main Deck monsters
  // that can ONLY be summoned by tributing 2+ (the classic
  // Egyptian-God-card shape: "cannot be Normal/Special Summoned except
  // by Tributing 2 monsters") are a real, if rare, accessibility cost
  // that the Extra Deck materials logic above never sees (these are
  // not Fusion/Synchro/Xyz/Link cards at all). Generic and reusable -
  // not specific to any one card.
  const tributeCountMatch = t.match(/tribut(?:e|ing) (\d+) monsters?/);
  const tributeCount = tributeCountMatch ? parseInt(tributeCountMatch[1], 10) : (costTribute ? 1 : 0);
  ...
  // Attack). ---
  // 2026-08-30 fix (Codex calibration audit, Maxx "C"): real oracle
  // text very commonly abbreviates "Life Points" as "LP" ("you gain
  // 1000 LP") - the spelled-out-only patterns missed this entire
  // common phrasing.
  const gainsLifePoints =
    /gain(?:s)? life points? equal to/i.test(t) ||
    /gain \d+ life points?/i.test(t) ||
    /gain(?:s)? \d+ lp\b/i.test(t) ||
    /gain(?:s)? lp equal to/i.test(t);
  ...
  // --- 2026-08-30 human calibration pass: four new reusable signals
  // (see docs/cardpool-classic-format-audit-2026-08-30.md for the
  // full brief this responds to) -----------------------------------

  // (B9) IMMEDIATE IMPACT vs delayed setup. "When/if this card is
  // summoned" is the standard oracle-text shape for an effect that
  // fires the instant the card hits the field - no further setup
  // needed. A delay phrase ("during your second Standby Phase", "the
  // turn after") is the opposite: real value, but not until a full
  // turn cycle has passed.
  const hasImmediateOnSummonImpact =
    /when this card is (?:normal |special )?summoned/i.test(t) ||
    /if this card is (?:normal |special )?summoned/i.test(t) ||
    /when this card is flip summoned/i.test(t);
  // "The turn after" needs a clause-level check, not a bare substring
  // match: "cannot attack the turn after you activate this effect" is
  // an extremely common drawback-on-an-otherwise-immediate-effect
  // phrasing (Jowgen the Spiritualist, among many real cards), not a
  // delayed-VALUE pattern at all - the effect itself already resolved
  // this turn, only a follow-up restriction lands next turn.
  const hasDelayedSetup =
    /during your (?:2nd|second|3rd|third|next) standby phase/i.test(t) ||
    text.split(/\.\s*/).some((sentence) => {
      const lower = sentence.toLowerCase();
      return (/the turn after/.test(lower) || /on the turn after/.test(lower)) && !/cannot attack/.test(lower);
    });

  // (E9) RANDOMNESS. A coin flip/die roll/random selection makes the
  // outcome unreliable - a real consistency cost, independent of how
  // powerful the possible outcomes are (the brief's own Arcana Force
  // EX ruler exception is handled as a manual override, not a
  // card-specific carve-out here - see the calibration report).
  const hasRandomness = /flip a coin/i.test(t) || /roll a (?:six-sided )?die/i.test(t) || /\brandomly\b/i.test(t);

  // (F9) SINGLE-USE vs repeatable. "Once per duel" (or an explicit
  // "can only be used once") is a real ceiling cap relative to an
  // otherwise-identical "once per turn" effect - the same interaction
  // most decks only ever get to use one time, not once per turn.
  const isSingleUseOnly = /once per duel/i.test(t) || /this effect can only be used once/i.test(t);
...
export function scoreCard(s, card, context = {}) {
  ...
  // 2026-08-30 human calibration pass (section 9F - "a strong
  // once-on-summon effect is generally worth less than a repeatable
  // oppressive effect, all else equal"): a hard "once per duel" cap
  // is a real ceiling reduction relative to an otherwise-identical
  // "once per turn" effect, which this file already treats as
  // effectively repeatable everywhere else.
  if (s.isSingleUseOnly) { power -= 0.6; reasons.push("effect is capped at once per duel, not repeatable"); }
  ...
  // 2026-08-30 human calibration pass (section 9A - "penalize cards
  // requiring 3+ materials... do not punish ordinary 2-material Xyz
  // or normal Fusion equally"): applies regardless of specificity, so
  // an otherwise-generic 3+ material summon (e.g. "3 Level 4
  // monsters") still costs a little real-world consistency even
  // though it earns no specificity penalty above.
  if (s.materials.materialCount >= 3) { dependency += 1.0; reasons.push(`requires ${s.materials.materialCount} materials to summon (more than an ordinary 2-material Xyz/Fusion)`); }
  ...
  // 2026-08-30 human calibration pass: each tribute BEYOND the first
  // is an additional, escalating accessibility cost (a 2-Tribute
  // monster is not merely "as hard as a 1-Tribute monster, twice" - it
  // is disproportionately harder to ever have 2 spare monsters AND
  // the tributed one(s) ready). Applies to any card, not just Extra
  // Deck ones - this is what actually catches the Egyptian-God-card
  // shape ("cannot be Summoned except by Tributing 2 monsters").
  if (s.tributeCount >= 2) { accessibility -= (s.tributeCount - 1) * 1.0; reasons.push(`requires Tributing ${s.tributeCount} monsters, not just 1`); }
  ...
  if (s.isExtraDeckCard) {
    accessibility -= 1.5;
    // 2026-08-30 human calibration pass (section 9A): the flat Extra
    // Deck tax above treated "2 generic monsters" and "3 specific
    // named monsters" identically for how easy the card is to
    // actually get onto the field - the calibration brief is explicit
    // that this is wrong ("do not punish ordinary 2-material Xyz or
    // normal Fusion equally" as a 3+/named-material summon). Named
    // materials get the largest additional tax (you need the actual
    // specific card, not just any monster of the right shape);
    // Attribute/Type/Tuner-constrained materials get a smaller one;
    // 3+ total materials (of any specificity) get a further tax on
    // top, since simply needing more pieces on hand at once is its
    // own real accessibility cost regardless of how narrow they are.
    if (s.materials.specificity === "named") { accessibility -= 1.0; reasons.push("requires specific named Extra Deck materials, not just any generic monsters"); }
    else if (s.materials.specificity === "constrained") { accessibility -= 0.6; }
    if (s.materials.materialCount >= 3) { accessibility -= 1.2; reasons.push(`requires ${s.materials.materialCount} materials to summon, not just 2`); }
  }
  ...
  // 2026-08-30 human calibration pass (section 9E - "coin toss / dice
  // / uncertain outcomes should reduce reliability"): a flat,
  // reusable consistency cost for any randomized outcome. Deliberately
  // NOT special-cased per card - the brief's own Arcana Force EX ruler
  // exception ("despite randomness, still Legendary because ultimate
  // impact and identity are exceptional") is handled as a manual
  // override, the same way the original 9 calibration anchors are,
  // rather than teaching the engine a name-specific carve-out.
  if (s.hasRandomness) { consistency -= 1.5; reasons.push("outcome depends on a coin flip/die roll/random selection"); }
  ...
  if (s.tributeCount >= 2) floor -= (s.tributeCount - 1) * 1.0;
  if (s.classifiedRefs.some((r) => r.type === "mandatory_target")) floor -= 1.0;
  // 2026-08-30 human calibration pass (section 9B - "reward cards
  // that... generate immediate board presence... function strongly
  // when topdecked"; "reduce valuation for cards whose effect only
  // matters after substantial delay"): floor is exactly the "what do
  // you get with zero setup, e.g. topdecked" axis, so immediate-vs-
  // delayed impact belongs here rather than in power (which measures
  // the effect's raw strength once it resolves, not when).
  if (s.hasImmediateOnSummonImpact) { floor += 0.6; reasons.push("delivers impact the moment it's summoned, not after further setup"); }
  if (s.hasDelayedSetup) { floor -= 1.0; reasons.push("effect is delayed by at least a full turn cycle before it matters"); }
...
export function proposeRarity(scores) {
  ...
  const legendaryGate =
    ...
    (
      ...
      //
      // 2026-08-30 human calibration pass: the ceiling >= 9.9 escape
      // was found to be the single largest source of false-Legendary
      // classifications - it let a maxed-out power/ceiling boss
      // monster reach Legendary on ceiling ALONE, with no floor check
      // at all, which is exactly the shape of card the calibration
      // brief repeatedly downgrades to Ultra/Secret (Van'Dalgyon the
      // Dark Dragon Lord, Number C106: Giant Red Hand, Number C88:
      // Gimmick Puppet Disaster Leo, Elemental HERO Chaos Neos -
      // "theoretical ceiling should not override practical
      // accessibility"). Gating the escape on dependency <= 4.0 keeps
      // it open for the brief's own stated exception shape - a
      // generic-materials Extra Deck monster (e.g. Arcana Force EX
      // ruler-style Xyz using a plain Level-based cost, not a named or
      // heavily Attribute/Type-constrained one) - while closing it for
      // named-material/archetype-locked bosses, whose dependency is
      // already pushed well past 4.0 by the existing named-materials
      // (+3.5) and archetype/mandatory-requirement penalties above.
      (ceiling >= 9.4 && (floor >= 3.0 || (ceiling >= 9.9 && dependency <= 4.0)))
    );
  if (legendaryGate) return "Legendary";

  //
  // 2026-09-01 go-live calibration pass (engine 2026-09-01.1): the
  // go-live spec targets ~300 Secret Rare and ~25-35 Legendary against
  // the final eligible cardpool. The last VERIFIED live run (engine
  // 2026-08-25.1, see the FORMAT-AWARE RECALIBRATION note above) found
  // only 51 Secret Rare against ~8,954 eligible cards - roughly 6x
  // short of target. This sandbox still has no live Supabase/network
  // access (see docs/cardpool-classic-format-audit-2026-08-30.md), so
  // the real yield of the loosened thresholds below has NOT been
  // re-verified against fresh live data since the 2026-08-30 pass
  // changed several upstream scoreCard() signals (tribute penalty,
  // removed flat Extra Deck bonus, dice/coin-toss and reward-card
  // handling) that would shift every card's draftValue/ceiling from
  // what the 2026-08-25 snapshot recorded. This is a DIRECTIONAL,
  // principled loosening within the same multi-axis gate shape (never
  // a blind top-N%/percentile cut) - draftValue and ceiling floors
  // each lowered by a moderate, documented amount rather than the gate
  // structure being changed - and MUST be verified with a live dry run
  // before being trusted as final:
  //   node scripts/audit-card-valuation.mjs
  // then check the printed Secret Rare / Legendary counts. If Secret
  // Rare lands far from ~300, adjust the two numeric thresholds just
  // below in +/-0.1-0.2 increments and re-run - do not reintroduce a
  // percentile/rank-based cut to hit the number faster.
  const secretGate =
    (
      // Path A: strong generic power AND meaningfully high ceiling.
      // Thresholds lowered from 5.75/7.0 (2026-08-25.1) to grow this
      // tier toward the ~300-card go-live target, but NOT as far as an
      // initial offline pass tried: replaying candidate thresholds
      // against the stale-but-structurally-similar 2026-08-25 score
      // snapshot (reports/card-valuation/2026-08-25T12-39-31-069Z/
      // full-proposal.json) against this file's own regression suite
      // found that draftValue>=5.05/ceiling>=6.1 (a first attempt,
      // ~300 cards offline) incorrectly pulled "Ancient Gear Beast"
      // (a generic 1-Tribute beatstick, human-approved and
      // correctness-tested as Ultra Rare, HUMAN_CALIBRATION_ROUND2 in
      // lib/valuation-engine.regression.test.mjs) up into Secret Rare.
      // 5.45/6.0 is the tightest offline estimate (~200-210 cards
      // against the stale snapshot) that keeps Ancient Gear Beast
      // (draftValue 5.29) excluded while still correctly promoting
      // Tragoedia (draftValue 5.65, ceiling 6.4) to Secret Rare,
      // matching its OWN human-approved value in the Codex calibration
      // table below (previously under-rated at Ultra Rare) - a real
      // improvement, not just a wider net. Given the 2026-08-30
      // scoreCard changes (tribute penalty, no flat Extra Deck bonus)
      // make the true live distribution stricter than this stale
      // snapshot, 200-ish is a more honest offline floor than 300; see
      // the block comment above this gate for the required live
      // verification step, and loosen further from here (never past
      // the point of re-admitting Ancient Gear Beast-shaped cards) if
      // the real run still falls well short of 300.
      (draftValue >= 5.45 && ceiling >= 6.0) ||
      // Path B: a real build-around payoff that's also reasonably
      // castable and not a floor disaster. Ceiling floor lowered from
      // 8.6 to 7.5, and accessibility floor relaxed from 4.0 to 3.0,
      // specifically so a powerful-but-costly payoff like "Obelisk the
      // Tormentor" (double-Tribute board wipe, accessibility 3.5,
      // ceiling 9.2 - human-approved Secret Rare, previously stuck at
      // Ultra Rare because it could clear neither the old Path A
      // draftValue bar nor Path B's stricter accessibility bar) can
      // reach Secret Rare on its ceiling without loosening Path A wide
      // enough to also catch Ancient Gear Beast-shaped generic
      // beatsticks. floor/genericUtility/dependency minimums are
      // unchanged.
      (ceiling >= 7.5 && floor >= 3.5 && genericUtility >= 3.5 && accessibility >= 3.0 && dependency <= 6.5)
    ) &&
    genericUtility >= 3.0;
...
// ---------------------------------------------------------
// CARD CONTEXT CLASSIFICATION (added 2026-08-30, Duelist Circle
// Classic architecture pass)
//
// WHY THIS EXISTS, AND WHY IT DOES NOT TOUCH scoreCard/proposeRarity
//
// The 2026-08-30 calibration audit found the automated engine
// systematically under-rates reactive/hand-trap cards relative to
// the Codex brief's human-judged approved table (see
// lib/valuation-engine.regression.test.mjs's "Codex calibration
// comparison" section) - and the explicit instruction that followed
// was to STOP chasing that gap with more specific regexes, because
// text-only valuation is fundamentally a baseline estimator, not a
// final authority, especially for exactly this kind of
// context-dependent value. This function is the alternative: instead
// of trying to make one blended draftValue number capture
// everything, it surfaces a SEPARATE, explicit axis - "what kind of
// card is this, structurally" - that a human reviewer (or a report)
// can use alongside draftValue, never folded back into it.
//
// This directly serves the brief's section 4 requirement: "a card
// extremely strong in one archetype should not automatically receive
// the same rarity as an equally strong card playable in almost every
// deck." draftValue's dependency penalty already pushes in that
// direction internally, but there was no explicit, reportable label
// separating "genuinely generic", "genuine archetype payoff",
// "narrow role-player", and the specific failure mode this project's
// own history has twice been burned by - a card that WEARS an
// archetype tag but actually plays as a splashable generic engine
// piece (Forbidden Droplet, Baronne de Fleur - see the v2 rewrite
// header comment at the top of this file). That fourth category is
// exactly why this is 4 buckets, not the 2 ("archetype" vs "generic")
// a simpler design would reach for.
//
// Buckets:
//   archetype         - a genuine, functional archetype payoff: the
//                        archetype tag is load-bearing in the card's
//                        own text (archetypeIsThematicOnly === false).
//                        A high ceiling here is EXPECTED and fine -
//                        this is what archetype identity looks like.
//   splashable_engine  - carries an archetype tag, but the tag is
//                        purely thematic (archetypeIsThematicOnly)
//                        AND the card is nonetheless broadly usable
//                        (high genericUtility, low dependency). This
//                        is the "engine splashability test" failure
//                        mode: a card that READS as archetype support
//                        but actually gets played as a generic
//                        staple regardless of archetype. Flag for
//                        rarity review the same way a genuinely
//                        generic card would be - the archetype tag on
//                        this kind of card should never be treated as
//                        a reason to go easier on it.
//   generic            - no archetype tag at all (or the tag is
//                        thematic-only), AND broadly usable
//                        (high genericUtility, low dependency). The
//                        honest, undisguised version of the case
//                        above - most classic generic staples land
//                        here (Giant Trunade, Harpie's Feather
//                        Duster).
//   narrow_support     - everything else: not a real archetype
//                        payoff, not broadly generic either - a
//                        situational, matchup-dependent, or simply
//                        low-generic-utility role-player. Several of
//                        the Codex brief's own approved calibration
//                        cards land here (Maxx "C", D.D. Crow, Battle
//                        Fader, Swift Scarecrow, Gorz) - matching the
//                        brief's own stated reasoning for each almost
//                        exactly (see the regression suite's
//                        calibration comment block), which is a good
//                        sign this classification is tracking a real,
//                        meaningful distinction rather than an
//                        arbitrary one.
//
// Thresholds (genericUtility >= 6, dependency <= 3) are the same
// "broadly usable" shape scoreCard already uses internally (e.g. the
// providesRemoval/genericUtility dependency gate) - not new, separately
// invented cutoffs.
// ---------------------------------------------------------

// NOTE: takes BOTH signals (for archetypeIsThematicOnly, computed by
// extractValuationSignals) AND scores (for genericUtility/dependency,
// computed by scoreCard - these are scoreCard OUTPUTS, not raw
// signals, since they already fold in the dependency-based penalties/
// bonuses this classification deliberately reuses rather than
// re-deriving). Always call as
// classifyCardContext(extractValuationSignals(card), scoreCard(signals, card), card).
export function classifyCardContext(signals, scores, card) {
  const hasArchetype = Boolean(card?.archetype);
  const isRealArchetypeLock = hasArchetype && signals.archetypeIsThematicOnly === false;

  if (isRealArchetypeLock) {
    return {
      context: "archetype",
      reason: `Genuine "${card.archetype}" payoff - the archetype is functionally load-bearing in this card's own text, not just a database tag.`,
    };
  }

  const broadlyUsable = scores.genericUtility >= 6 && scores.dependency <= 3;

  if (broadlyUsable && hasArchetype) {
    return {
      context: "splashable_engine",
      reason: `Carries a "${card.archetype}" tag but the tag is thematic-only, and the card itself is broadly usable outside that archetype (genericUtility ${scores.genericUtility}, dependency ${scores.dependency}) - treat like a generic card for rarity purposes, not an archetype payoff.`,
    };
  }
  ...
  if (broadlyUsable) {
    return {
      context: "generic",
      reason: `No functional archetype requirement, broadly usable (genericUtility ${scores.genericUtility}, dependency ${scores.dependency}).`,
    };
  }

  return {
    context: "narrow_support",
    reason: `Not a functional archetype payoff, and not broadly usable either (genericUtility ${scores.genericUtility}, dependency ${scores.dependency}) - a situational or matchup-dependent role-player.`,
  };
}
...
// PROPOSAL columns (proposed_game_rarity and the 8 scores)
// specifically. rarity_manually_overridden is the analogous flag for
// the LIVE game_rarity column itself (see
// scripts/apply-manual-rarity-overrides.mjs, added 2026-08-30, which
// is the first live code to actually read it - previously only the
// now-deprecated scripts/classify-rarities.mjs did, making it a
// dormant guard on nothing).
...
// ---------------------------------------------------------
// ARCHETYPE RELEVANCE HINTS (2026-08-30 human calibration pass,
// section 9G)
//
// "There are only 3 players and they strongly value nostalgia...
// allow a manually-maintained archetype relevance / popularity
// weighting rather than trying to infer player nostalgia from card
// text. Do NOT hardcode arbitrary assumptions across thousands of
// archetypes... Do not automatically increase rarity yet solely
// because of this flag. Use it as context for review and expected
// play frequency."
//
// This is DELIBERATELY not read by scoreCard or proposeRarity
// anywhere - it is a small, explicit, human-maintained lookup that
// the audit output surfaces as its own field alongside context/
// rarity, for a human reviewer to weigh. Extending it to cover
// another archetype is a one-line data change to the object below,
// never a new code path - that is the "clean configuration
// mechanism" the brief asked for, kept intentionally minimal rather
// than a general popularity-inference subsystem.
// ---------------------------------------------------------
export const ARCHETYPE_RELEVANCE_HINTS = {
  "Elemental HERO": "high",
  HERO: "high",
  "Dark Magician": "high",
  "Magician Girl": "high",
...
export function getArchetypeRelevanceHint(archetype) {
  if (!archetype) return "unknown";
  const lower = archetype.toLowerCase();
  const hit = Object.keys(ARCHETYPE_RELEVANCE_HINTS).find(
    (key) => lower === key.toLowerCase() || lower.includes(key.toLowerCase())
  );
  return hit ? ARCHETYPE_RELEVANCE_HINTS[hit] : "unknown";
}

// 2026-09-01 go-live calibration pass: Secret Rare gate thresholds
// loosened (draftValue/ceiling floors lowered in both paths) to move
// the tier population toward the go-live spec's ~300-card target -
// see the inline note directly above secretGate in proposeRarity()
// for the exact deltas and the required live-verification step. The
// Legendary gate itself was deliberately left untouched: the last
// verified count (25, engine 2026-08-25.1) already sits at the low
// end of the target 25-35 band, and further loosening a gate that
// took an iterative empirical grid search to calibrate risks
// overshooting past 35 without a fresh live run to check against. If
// a live dry run comes back under 25 (plausible - the 2026-08-30 pass
// closed the old ceiling>=9.9 no-floor escape, which likely lowered
// the real live count below this stale 25), the documented fallback
// is a NARROWER reopening of that escape (e.g. dependency <= 3.0
// instead of the removed unconditional version) rather than reverting
// the 2026-08-30 fix outright - see that gate's own inline history.
export const VALUATION_ENGINE_VERSION = "2026-09-01.1";
