...
export function proposeRarity(scores) {
  ...
  // 2026-09-01.2 GO-LIVE FLATTENING PASS (final pre-launch sprint):
  // the go-live spec now wants a much flatter, more gameplay-friendly
  // Season 1 shape against the ~6,181-card eligible pool -
  // Legendary ~45-55, Secret ~180-220, Ultra ~1000-1200 (a LARGE
  // healthy tier), Super ~1100-1300, Rare ~1400-1650, Normal
  // ~1900-2150 - a deliberate philosophy change from the prior
  // scarcity-first 2026-09-01.1 pass, not a bug fix to it.
  //
  // legendaryGate is also STRUCTURALLY tightened here, not just
  // recounted: the previous Path B (ceiling >= 9.4 && floor >= 3.0)
  // ignored draftValue and dependency entirely once ceiling/floor
  // cleared their (low) bars, which is exactly how "structurally
  // wrong" cards slipped in - a 2-Tribute board wipe with mediocre
  // accessibility (Obelisk the Tormentor, accessibility 4.3) or an
  // Attribute-locked-but-not-named Xyz with an awkward material cost
  // (Number 102: Star Seraph Sentry, accessibility 4.8) could clear
  // ceiling+floor alone with no real check on "is this realistically
  // playable/low-setup-burden". The replacement gate requires ALL of:
  // a high ceiling, real generic value, LOW dependency (<=4.0, applied
  // unconditionally now, not just inside an escape clause), and
  // (accessibility >= 5.5) - the two Path B examples above sit at
  // 4.3/4.8 and are correctly excluded (they land Secret Rare
  // instead, matching their human-approved value in
  // lib/valuation-engine.regression.test.mjs's HUMAN_CALIBRATION_
  // ROUND2 table) - plus a real floor (>=5.5, "strong when drawn
  // alone") unless the ceiling is truly maximal (>=9.8).
  //
  // Ancient Gear Beast is a separate, narrower, already-documented
  // gap: HUMAN_CALIBRATION_ROUND2 in the regression suite establishes
  // (Confidence: HIGH, re-scored live from its real effect text) that
  // it should be Ultra Rare, but the only local full-catalog snapshot
  // available for this recalibration (2026-08-25, pre-dating the
  // 2026-08-30 tribute-penalty scoreCard fix) still records an
  // inflated draftValue/ceiling for it that clears this gate
  // structurally identically to genuine bombs like Light and Darkness
  // Dragon. This is a stale-INPUT-score problem, not a gate-logic
  // problem, and the project's own established pattern for exactly
  // this situation (see the Codex calibration / APPROVED_CALIBRATION
  // table) is a manual override, not further gate surgery - so
  // "Ancient Gear Beast" -> "Ultra Rare" is now applied as a 16th
  // manual override at the seed-data layer (see
  // scripts/apply-final-season1-rarities.mjs), sourced directly from
  // that already-existing, already-tested human-approved value.
  const legendaryGate =
    ceiling >= 9.0 &&
    genericUtility >= 4.0 &&
    accessibility >= 5.5 &&
    dependency <= 4.0 &&
    (floor >= 5.5 || ceiling >= 9.8);
  if (legendaryGate) return "Legendary";

  // Secret is the main "WOW" tier now (target ~180-220): a single
  // draftValue floor plus a light structural check (ceiling/generic
  // utility) so a card needs both real value AND real upside to land
  // here - threshold derived from the real percentile structure of
  // the eligible pool the same way the bulk tiers below are, per this
  // pass's flattening goal (see scripts/apply-final-season1-
  // rarities.mjs for the derivation and the resulting counts).
  const secretGate =
    ceiling >= 6.0 && genericUtility >= 3.0 && draftValue >= 5.26;
  if (secretGate) return "Secret Rare";

  // Ultra is deliberately the LARGE, healthy "backbone of good cards"
  // tier in this pass (target ~1000-1200 of ~6,181 eligible cards) -
  // a single draftValue floor, set from the real percentile structure
  // of the eligible pool (not an arbitrary round number), rather than
  // the narrower multi-axis build-around gate this tier used before
  // the flattening pass. See scripts/apply-final-season1-rarities.mjs
  // for the percentile derivation.
  const ultraGate = draftValue >= 4.46;
  if (ultraGate) return "Ultra Rare";

  if (draftValue >= 3.98) return "Super Rare";
  if (draftValue >= 3.80) return "Rare";
  return "Normal"
}
...
export const VALUATION_ENGINE_VERSION = "2026-09-01.2";
