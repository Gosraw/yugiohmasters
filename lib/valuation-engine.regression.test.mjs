...
test("Codex calibration cards: document current engine output for each (regression pin, not a correctness claim)", () => {
  ...
  // engine tuning).
  // 2026-09-01 go-live Secret Rare recalibration (lib/valuation-
  // engine.mjs proposeRarity(), secretGate) moved Tragoedia from
  // Ultra Rare to Secret Rare - which now MATCHES its own approved
  // value above, a genuine improvement (one fewer card needing the
  // valuation_manually_overridden workaround), not a regression.
...
test("HUMAN CALIBRATION ROUND 2: document current post-improvement engine output for each of the 8 verified cards", () => {
  // 2026-09-01 go-live Secret Rare recalibration (secretGate Path B:
  // ceiling floor lowered 8.6->7.5, accessibility floor relaxed
  // 4.0->3.0) moved Obelisk the Tormentor from Ultra Rare to Secret
  // Rare - which now MATCHES its own approved value above (a costly
  // but genuinely exceptional double-Tribute payoff), a genuine
  // improvement. Ancient Gear Beast deliberately stays Ultra Rare -
  // the same recalibration pass was explicitly tuned (see
  // valuation-engine.mjs's secretGate comments) to grow Secret Rare
  // toward the go-live target WITHOUT re-admitting this specific
  // generic-beatstick shape.
